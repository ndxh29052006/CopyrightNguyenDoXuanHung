...
module.exports.handleEvent = async ({api, event}) => {
    ...
    let name = user[event.senderID].name
      ...
      //Khai báo admin bot
    var admin = "100000317124060" //Thay uid adminbot :> ????
    if(event.senderID == api.getCurrentUserID()) return;
    if (data[event.body]) {
      return api.sendMessage({
        body: ` ${name} vi phạm từ ${event.body}?\n Chửi bot con cặc gì vậy bạn`,
        mentions:[{
                tag:name, 
                id:event.senderID
            }]
...
module.exports.run = async function({api, args, event}) {
    ...
    if (!content) return api.sendMessage(`Con cặc xinh đẹp`,event.threadID, event.messageID)
    var data = JSON.parse(fs.readFileSync(path.join(__dirname, "./cache/badwords.json"), {encoding: "utf8"}))
    if (!args[0])return api.sendMessage(`Dùng: \nbadword add [từ ngữ]\n\nAuthor: BuiChiThong`,event.threadID,event.messageID)
    if (args[0] == `add`){
